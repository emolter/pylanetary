...
class Spectrum:
  ...
  your_spec = spectrum(wl,spec,rest_f)
  
  Fit the spectrum with a Lorentian model and plot the results:
  peak, std, = test.fit_profile(fit_type = 'Lorentzian', showplots = 'y', RMS = 0.017530593)
  
  Calculate the Doppler velcoity using the calculated Lorentzian model:
  wind, err = test.calc_doppler_vel(rest_freq = 345.79598990, peak = peak, err = std)
  '''
  
  ###############################################################################
  
  def __init__(self, spec, rest_f, wl = None, freq = None):
    self.wl = wl
    self.freq = freq
    ...
    self.spec = spec # attribute, spectral inensity axis
    self.rest_f = rest_f # attribute, rest frequency of the molecular transition
    self.wl_res = self.wl[1] - self.wl[0] # attribute, wavelength resolution
    self.freq_res = self.freq[1] - self.freq[0] # attrbute, frequency resolution
    self.chan_num = np.size(self.wl) # attribute, number of wavelength/frequency channels
	
  ###############################################################################
  
  def fit_profile(self, fit_type, showplots, initial_fit_guess = None, RMS = None):
    '''
    Parameters
    ----------
    fit_type - the type of line shape used to fit the spectra, string. Options are:
        All: Fit the spectrum with all line 5 line shape options and plot for visual comparison. Data is always plotted
    ...
    if RMS == None:
      RMS,median,rms_std = sigma_clipped_stats(self.spec)
    ...
    if fit_type != 'All':
      ...
      a0.errorbar(self.freq,self.spec,yerr=RMS, fmt='o',label='Original Data')
      for key in model:
        ...
        residuals = self.spec - result.best_fit
        print(result.fit_report())
        # Plot the results
        a0.plot(self.freq,result.best_fit,'-',label = model[key]['label'],color = model[key]['color'])
        a1.plot(self.freq,residuals,'--',color = model[key]['color'])
        # End of for loop
      a1.set_xlabel('Frequency (GHz)')
      a0.set_ylabel('Intensity (mJy)')
      a1.set_ylabel('Residuals')
      a0.title.set_text('Comparison of Line Shapes for a Pressure Broadened Atmosphere')
      a0.legend()
      plt.show()
    else:
    ...
    if fit_type != 'All':
      chi = np.sum(((np.subtract(self.spec,fit_result.best_fit)/RMS))**2)
      chi_red = chi/(self.chan_num - 4)
    else:
      chi = np.NAN
      chi_red = np.NAN

    if showplots == 'y' or showplots == 'Y':
      plt.vlines(x = self.rest_f, ymin=0, ymax=np.max(self.spec), color = 'grey', linestyles='dashed', label='Rest Frequency')
      plt.errorbar(self.freq,self.spec,yerr=RMS, fmt='o',label='Original Data')
      plt.plot(self.freq,fit_result.best_fit,'-',label = model[fit_type]['label'])
  ...
  def calc_doppler_vel(self, peak, err): #, observed_freq_uncertainty=0.0):
    '''
    Parameters
    ---------- 
    ...
    err = np.abs(a*std) # Calculate the error in velocity
...
# Simple single spectra testing case
  ...
  #test = Spectrum(spec, freq = freq,rest_f = 345.79598990)
  #test = Spectrum(spec, freq = freq) #HCN
  #test_peak, test_std, chi, chi_red = test.fit_profile(fit_type = 'Moffat', showplots = 'y')
