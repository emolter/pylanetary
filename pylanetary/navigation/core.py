...
def lat_lon(
    ...
    target_proj = ccrs.Orthographic(
        central_longitude=ob_lon,
        central_latitude=ob_lat,
        globe=img_globe)

    # Define the input and output grids. source is in lat/lon, target is in km
    target_ny, target_nx = int(shape[0]), int(shape[1])
    xx, yy = np.arange(target_nx) - target_nx / 2 - \
        dx, np.arange(target_ny) - target_ny / 2 - dy
    target_x_points, target_y_points = np.meshgrid(
        xx * pixscale_km, yy * pixscale_km)

    # Project the longitude and latitude grids
    source_nx, source_ny = int(360 / deg_per_px), int(180 / deg_per_px)
    source_x_coords, source_y_coords, extent = mesh_projection(
        source_proj, source_nx, source_ny, x_extents=(0, 360), y_extents=(-90, 90))
    lon_grid = regrid(
        source_x_coords,
        source_x_coords,
        source_y_coords,
    ...
    lon_grid = rotate_about_pivot(lon_grid.filled(
        np.nan), -np_ang, (int(np.round(target_nx / 2 + dx)), int(np.round(target_ny / 2 + dy))))
    lat_grid_g = rotate_about_pivot(lat_grid_g.filled(
        np.nan), -np_ang, (int(np.round(target_nx / 2 + dx)), int(np.round(target_ny / 2 + dy))))
    lat_grid_c = np.rad2deg(
        np.arctan((1 - (r_e - r_p) / r_e)**2 * np.tan(np.deg2rad(lat_grid_g))))
...
def emission_angle(ob_lat, surf_n):
    ...
    ob = np.asarray([np.cos(np.radians(ob_lat)),
                    0, np.sin(np.radians(ob_lat))])
...
def limb_darkening(mu, a, law='exp', mu0=None):
    ...
    # necessary so when floats are passed in, the line mu[bad] = 0 doesnt
    # complain about indexing a float
    mu = np.array(mu)
    idnan = np.isnan(mu)
    mu[idnan] = 0.0
    ...
    if law == 'disc' or law == 'disk':
        ...
        raise ValueError(
            'limb darkening laws accepted: "disc", "linear", "exp", "minnaert", "quadratic", "square-root" ')
...
class ModelEllipsoid:
    ...
    def __init__(
        ...
        if shape is None:
            ...
            pad_width = 10
            sz = int(
                2 * np.ceil(np.max([req, rpol]) / pixscale_km) + pad_width)
            shape = (sz, sz)
        if sun_lon is None:
            self.sun_lon = ob_lon
        else:
        ...
        self.lat_g, self.lat_c, self.lon_w, self.image_grid_km_x, self.image_grid_km_y = lat_lon(
            shape, pixscale_km, ob_lon, ob_lat, np_ang, req, rpol, dx=center[0], dy=center[1], longitude_convention=longitude_convention)
...
class ModelBody(ModelEllipsoid):
    ...
    def __init__(self, body, pixscale, shape=None):
        ...
        self.pixscale_km = self.ephem['delta'] * \
            u.au.to(u.km) * np.radians(self.pixscale_arcsec / 3600.)
...
class Nav(ModelBody):
    ...
    def colocate(
        ...
        * sometimes dxerr, dyerr give unrealistic or undefined behavior
        ...
        if (mode == 'convolution') or (mode == 'disk'):
            ...
            model = convolve_with_beam(
                model, kwargs['beam'], mode=kwargs['psf_mode'])
            data_to_compare = self.data
        elif mode == 'canny':
            model_planet = self.ldmodel(
                kwargs['tb'], kwargs['a'], law=kwargs['law'])
            if kwargs['beam'] is not None:
                model_planet = convolve_with_beam(
                    model_planet, kwargs['beam'], mode=kwargs['psf_mode'])
        ...
        [dx, dy, dxerr, dyerr] = chi2_shift(
            model, data_to_compare, err=kwargs['err'])

        if diagnostic_plot:

            model_shifted = shift2d(model, dx, dy)
            fig, ax = colocate_diagnostic_plot(
                model_shifted, data_to_compare, mode)
    ...
    def xy_shift_model(self, dx, dy):
        ...
        self.lat_g, self.lat_c, self.lon_w, self.image_grid_km_x, self.image_grid_km_y = lat_lon(
            shape, self.pixscale_km, self.ob_lon, self.ob_lat, self.np_ang, self.req, self.rpol, dx=dx, dy=dy)
        self.surf_n = surface_normal(self.lat_g, self.lon_w, self.ob_lon)
        self.mu = emission_angle(self.ob_lat, self.surf_n)
    ...
    def write(self, outstem, header={}, flux_unit=''):
        ...
        # gives priority to input header dict
        header_out = {**header_default, **header}
    ...
    def reproject(self, projection='equirectangular', shape=None):
        ...
        img_globe = ccrs.Globe(
            semimajor_axis=self.req,
            semiminor_axis=self.rpol,
            ellipse=None)
        source_proj = ccrs.Orthographic(
            central_longitude=self.ob_lon,
            central_latitude=self.ob_lat,
            globe=img_globe)
        ...
        if projection == 'equirectangular':
            ...
            target_proj = ccrs.PlateCarree(
                central_longitude=180 - self.ob_lon, globe=img_globe)
            ...
            if shape is None:
                ...
                target_ny = int(np.round(180 / self.deg_per_px))
            target_proj = ccrs.Orthographic(
                central_longitude=180 - self.ob_lon,
                central_latitude=np.sign(
                    self.ob_lat) * 90,
                globe=img_globe)
            ...
            if shape is None:
                raise ValueError(
                    'shape must be specified if target_projection is not equirectangular or polar')
        ...
        # support elliptical transformations, only circular. see docstring of
        # lat_lon()
        yc, xc = np.unravel_index(
            np.argmin(
                self.image_grid_km_x**2 + self.image_grid_km_y**2), self.image_grid_km_x.shape)
        ...
        target_x_points, target_y_points, extent = mesh_projection(
            target_proj, target_nx, target_ny)
        projected = regrid(
            data_rot,
            self.image_grid_km_y,
            self.image_grid_km_x,
            source_proj,
            target_proj,
            target_x_points,
            target_y_points,
        ...
        return projected.filled(
            np.nan), mu_projected.filled(
            np.nan), mu0_projected.filled(
            np.nan), target_x_points, target_y_points
