...
def project_ellipse_double(a, e, params_ring, params_sys, origin=np.array([
                           0, 0, 0]), n=50, proj_plane=[0, 0, 1]):
    '''
    ...
    # project these
    a_proj, b_proj = plane_project(
        a_rot, proj_plane), plane_project(
        b_rot, proj_plane)
...
def ring_area(a, e, width, delta_width=0.0, B=90.0):
    ...
    e_i = c_i / a_i
    e_o = c_o / a_o
    A = np.pi * a_o**2 * np.sqrt(1 - e_o**2) - \
        np.pi * a_i**2 * np.sqrt(1 - e_i**2)
...
class Ring:
    ...
    * Passing astropy Quantities is not yet tested
    ...
    def as_2d_array(
        ...
        ann, params_out = self.as_elliptical_annulus(
            focus, pixscale, width, return_params=True)
    ...
    * lookup table to avoid needing to specify both location and horizons_loc
    ...
    def __init__(self,
        ...
        if fluxes == 'default':
            ...
            # lazy fix of bad table data, re-think this later
            taus[np.isnan(taus)] = 1.0
            fluxes = 1 - np.exp(-taus)

        # instantiate ring objects for all the rings
        params_sys = [
            90.0,
            self.systemtable['opening_angle'].value -
            90.0,
            self.np_ang]  # [w, i, omega]
        self.rings = {}
